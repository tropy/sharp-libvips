...
cd ${DEPS}/orc
CFLAGS="${CFLAGS} -O3" LDFLAGS=${LDFLAGS/\$/} meson setup _build --default-library=${TYPE} --buildtype=release --strip --prefix=${TARGET} ${MESON} \
...
if [ ! "$ELECTRON" = true ]; then
  ...
  ./configure --host=${CHOST} --prefix=${TARGET} ${TYPE_FLAGS} --disable-dependency-tracking \
    ...
    ${DARWIN:+--sysconfdir=$(brew --prefix)/etc} --disable-docs
  make install-strip
