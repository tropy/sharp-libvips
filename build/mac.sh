...
function copydeps {
  ...
  for dep in $(otool -LX $file | awk '{print $1}' | grep $(brew --prefix)); do
...
copydeps $(brew --prefix libvips)/lib/libvips-cpp.42.dylib lib
copydeps $(brew --prefix poppler8)/lib/libpoppler.91.dylib lib
